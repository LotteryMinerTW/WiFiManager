...
"<form action='/wifi'    method='get'><button>設定WIFI</button></form><br/>\n", // MENU_WIFI
"<form action='/0wifi'   method='get'><button>設定WIFI (無掃描)</button></form><br/>\n", // MENU_WIFINOSCAN
...
"<form action='/erase'   method='get'><button class='D'>抹除資料</button></form><br/>\n", // MENU_ERASE
...
const char HTTP_FORM_WIFI[]        PROGMEM = "<label for='s'>SSID</label><input id='s' name='s' maxlength='32' autocorrect='off' autocapitalize='none' placeholder='{v}'><br/><label for='p'>密碼</label><input id='p' name='p' maxlength='64' type='password' placeholder='{p}'><input type='checkbox' id='showpass' onclick='f()'> <label for='showpass'>顯示密碼</label><br/>";
...
const char HTTP_SAVED[]            PROGMEM = "<div class='msg'>正在儲存登入憑證並嘗試將ESP連結至網路<br/>若連接失敗，請重新設定AP並再試一次</div>";
const char HTTP_PARAMSAVED[]       PROGMEM = "<div class='msg S'>已儲存<br/></div>";
const char HTTP_END[]              PROGMEM = "</div></body></html>";
const char HTTP_ERASEBTN[]         PROGMEM = "<br/><form action='/erase' method='get'><button class='D'>清除WiFi設定</button></form>";
...
const char HTTP_STATUS_ON[]        PROGMEM = "<div class='msg S'><strong>已連接到</strong>{v}<br/><em><small>IP為 {i}</small></em></div>";
const char HTTP_STATUS_OFF[]       PROGMEM = "<div class='msg {c}'><strong>未連接到/strong>{v}{r}</div>"; // {c=class} {v=ssid} {r=status_off}
const char HTTP_STATUS_OFFPW[]     PROGMEM = "<br/>驗證失敗"; // STATION_WRONG_PASSWORD,  no eps32
const char HTTP_STATUS_OFFNOAP[]   PROGMEM = "<br/>找不到該AP";   // WL_NO_SSID_AVAIL
...
const char HTTP_HELP[] PROGMEM =
	...
	"<td>顯示WiFi掃描的結果與進入WiFi設定(/0wifi noscan)</td></tr>"
	"<tr><td><a href='/wifisave'>/wifisave</a></td>"
	"<td>儲存WiFi設定資訊與設定裝置。須提供參數。</td></tr>"
	...
	"<td>關閉\"captiveportal\"彈出視窗(登入WiFi)，設定頁面將保持可用</td></tr>"
	"<tr><td>/exit</td>"
	"<td>離開設定頁面，設定頁面將不再可用</td></tr>"
	"<tr><td>/restart</td>"
	"<td>重新啟動裝置</td></tr>"
	"<tr><td>/erase</td>"
	"<td>清除WiFi設定並重新啟動裝置。在輸入新的WiFi設定資料之前，設備不會重新連接到網路。</td></tr>"
	"</table>"
	"<p/>原始Github專案 <a href='https://github.com/tzapu/WiFiManager'>tzapu/WiFiManager</a>，由<a href='https://github.com/000hen'>Muisnow</a>漢化";
#else
const char HTTP_HELP[]             PROGMEM = "";
#endif

const char HTTP_UPDATE[] PROGMEM = "更新韌體<br/><form method='POST' action='u' enctype='multipart/form-data' onchange=\"(function(el){document.getElementById('uploadbin').style.display = el.value=='' ? 'none' : 'initial';})(this)\"><input type='file' name='update' accept='.bin,application/octet-stream'><button id='uploadbin' type='submit' class='h D'>更新</button></form><small><a href='http://192.168.4.1/update' target='_blank'>* 可能無法在\"captive portal\"中正確運行，因此建議您於瀏覽器中開啟 http://192.168.4.1</a><small>";
const char HTTP_UPDATE_FAIL[] PROGMEM = "<div class='msg D'><strong>更新失敗!</strong><Br/>請重新啟動再試一次</div>";
const char HTTP_UPDATE_SUCCESS[] PROGMEM = "<div class='msg S'><strong>更新成功</strong><br/>裝置即將重啟...</div>";
...
// @todo remove html elements from progmem, repetetive strings
	...
	const char HTTP_INFO_chiprev[]    PROGMEM = "<dt>Chip rev</dt><dd>{1}</dd>";
  	...
  	const char HTTP_INFO_aphost[]     PROGMEM = "<dt>存取點主機名稱</dt><dd>{1}</dd>";
...
const char HTTP_INFO_memsketch[]  PROGMEM = "<dt>記憶體 - Sketch size</dt><dd>已使用 / 全部 bytes<br/>{1} / {2}";
const char HTTP_INFO_freeheap[]   PROGMEM = "<dt>記憶體 - Free heap</dt><dd>{1} bytes 可用</dd>";
const char HTTP_INFO_wifihead[]   PROGMEM = "<br/><h3>WiFi</h3><hr>";
const char HTTP_INFO_uptime[]     PROGMEM = "<dt>啟動時間</dt><dd>{1} mins {2} secs</dd>";
const char HTTP_INFO_chipid[]     PROGMEM = "<dt>Chip ID</dt><dd>{1}</dd>";
const char HTTP_INFO_idesize[]    PROGMEM = "<dt>閃存大小</dt><dd>{1} bytes</dd>";
const char HTTP_INFO_sdkver[]     PROGMEM = "<dt>SDK版本</dt><dd>{1}</dd>";
const char HTTP_INFO_cpufreq[]    PROGMEM = "<dt>CPU速度</dt><dd>{1}MHz</dd>";
const char HTTP_INFO_apip[]       PROGMEM = "<dt>存取點IP</dt><dd>{1}</dd>";
const char HTTP_INFO_apmac[]      PROGMEM = "<dt>存取點MAC</dt><dd>{1}</dd>";
const char HTTP_INFO_apssid[]     PROGMEM = "<dt>存取點SSID</dt><dd>{1}</dd>";
const char HTTP_INFO_apbssid[]    PROGMEM = "<dt>BSSID</dt><dd>{1}</dd>";
const char HTTP_INFO_stassid[]    PROGMEM = "<dt>站點SSID</dt><dd>{1}</dd>";
const char HTTP_INFO_staip[]      PROGMEM = "<dt>站點IP</dt><dd>{1}</dd>";
const char HTTP_INFO_stagw[]      PROGMEM = "<dt>站點gateway</dt><dd>{1}</dd>";
const char HTTP_INFO_stasub[]     PROGMEM = "<dt>站點subnet</dt><dd>{1}</dd>";
const char HTTP_INFO_dnss[]       PROGMEM = "<dt>DNS伺服器</dt><dd>{1}</dd>";
const char HTTP_INFO_host[]       PROGMEM = "<dt>Hostname</dt><dd>{1}</dd>";
const char HTTP_INFO_stamac[]     PROGMEM = "<dt>站點MAC</dt><dd>{1}</dd>";
...
const char S_titlewifisaved[]     PROGMEM = "憑證已儲存";
const char S_titlewifisettings[]  PROGMEM = "設定已儲存";
const char S_titlewifi[]          PROGMEM = "設定ESP";
...
const char S_nonetworks[]         PROGMEM = "沒有找到可用的網路。刷新來再次掃描";
const char S_staticip[]           PROGMEM = "靜態IP";
const char S_staticgw[]           PROGMEM = "靜態網關(Gateway)";
const char S_staticdns[]          PROGMEM = "靜態DNS";
const char S_subnet[]             PROGMEM = "子網路遮罩";
const char S_exiting[]            PROGMEM = "正在離開";
const char S_resetting[]          PROGMEM = "模組將在幾秒鐘內重置。";
const char S_closing[]            PROGMEM = "您可以關閉這個頁面，登入頁面將會繼續執行。";
const char S_error[]              PROGMEM = "發生錯誤";
const char S_notfound[]           PROGMEM = "檔案未找到\n\n";
